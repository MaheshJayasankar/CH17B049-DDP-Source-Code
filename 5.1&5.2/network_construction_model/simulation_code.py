...
class Network:
        ...
        if val == 0:
            ...
            if len(velocity_arr) == 0:
                ...
                for key in self.branch_drp_time:
                    self.branch_drp_time[key] = self.branch_drp_time[key] + self.delt_Tmin_val
                for key in self.branch_drp_pstn:
                    print("acha")
                    length_branches = self.calc_length_branches()
                    if self.Branch_velocity[key] * self.Branch_velocity_prev[key] > 0:
                        self.branch_drp_pstn[key] = self.branch_drp_pstn[key] + self.delt_Tmin_val * abs(self.Branch_velocity[key]) / \
                                               length_branches[key]
                    else:
                        ...
                        self.branch_drp_pstn[key] = self.branch_drp_pstn[key] + self.delt_Tmin_val * abs(self.Branch_velocity[key]) / \
                                               length_branches[key]
                moving_branch_from = node_or_branch
                print(moving_branch_from)
                moving_branch_to = self.branch_cnctvty_branchs_cnctd[node_travelling_from]
                ...
                for jdx in range(len(moving_branch_to)):
                    if moving_branch_to[jdx] == moving_branch_from:
                        print("help")
                        if moving_branch_from > 0:
                            self.branch_drp_pstn[moving_branch_to[jdx]] = np.insert(self.branch_drp_pstn[moving_branch_to[jdx]], -1, 1)
                            self.branch_drp_type[moving_branch_to[jdx]] = np.insert(self.branch_drp_type[moving_branch_to[jdx]], -1, self.branch_drp_type[moving_branch_from][-1])
                            self.branch_drp_time[moving_branch_to[jdx]] = np.insert(self.branch_drp_time[moving_branch_to[jdx]], -1, self.branch_drp_time[moving_branch_from][-1])
                            self.branch_drp_num[moving_branch_to[jdx]] = np.insert(self.branch_drp_num[moving_branch_to[jdx]], -1, self.branch_drp_num[moving_branch_from][-1])
                    elif moving_branch_to[jdx] != moving_branch_from:
                        self.branch_drp_pstn[moving_branch_to[jdx]] = np.insert(self.branch_drp_pstn[moving_branch_to[jdx]], 0, 0)
                        self.branch_drp_type[moving_branch_to[jdx]] = np.insert(self.branch_drp_type[moving_branch_to[jdx]], 0,
                                                                           self.branch_drp_type[moving_branch_from][-1])
                        self.branch_drp_time[moving_branch_to[jdx]] = np.insert(self.branch_drp_time[moving_branch_to[jdx]], 0,
                                                                           self.branch_drp_time[moving_branch_from][-1])
                        self.branch_drp_num[moving_branch_to[jdx]] = np.insert(self.branch_drp_num[moving_branch_to[jdx]], 0,
                ...
                self.decision['branch'], self.decision['node_num'] = self.drop_movement(node_travelling_from, 3)"""
                        ...
                        if len(decision['branch']) == 0:
                           ...
                           # print(tochoose_moving_branch)
                            ...
                            for ldx in range(len(moving_branch_from)):
                                ...
                                """if len(self.branch_drp_type[moving_branch_from[ldx]]) > 1:

                                    if self.Network_solution[self.node_indices[node_from]] - self.Network_solution[self.node_indices[node_options[ldx]]] < 0:

                                        self.branch_drp_pstn[moving_branch_from[ldx]] = np.delete(self.branch_drp_pstn[moving_branch_from[ldx]], -1)
                                        self.branch_drp_type[moving_branch_from[ldx]] = np.delete(self.branch_drp_type[moving_branch_from[ldx]], -1)
                                        self.branch_drp_time[moving_branch_from[ldx]] = np.delete(self.branch_drp_time[moving_branch_from[ldx]], -1)
                                        self.branch_drp_num[moving_branch_from[ldx]] = np.delete(self.branch_drp_num[moving_branch_from[ldx]], -1)
                                    elif self.Network_solution[self.node_indices[node_from]] - self.Network_solution[self.node_indices[node_options[ldx]]] > 0:
                                        self.branch_drp_pstn[moving_branch_from[ldx]] = np.delete(self.branch_drp_pstn[moving_branch_from[ldx]], 0)
                                        self.branch_drp_type[moving_branch_from[ldx]] = np.delete(self.branch_drp_type[moving_branch_from[ldx]], 0)
                                        self.branch_drp_time[moving_branch_from[ldx]] = np.delete(self.branch_drp_time[moving_branch_from[ldx]], 0)
                                        self.branch_drp_num[moving_branch_from[ldx]] = np.delete(self.branch_drp_num[moving_branch_from[ldx]], 0)
                                else:
                                    self.branch_drp_pstn[moving_branch_from[ldx]] = np.delete(self.branch_drp_pstn[moving_branch_from[ldx]], 0)
                                    self.branch_drp_type[moving_branch_from[ldx]] = np.delete(self.branch_drp_type[moving_branch_from[ldx]], 0)
                                    self.branch_drp_time[moving_branch_from[ldx]] = np.delete(self.branch_drp_time[moving_branch_from[ldx]], 0)
                ...
                if len(self.decision['branch']) == 2:
                    ...
                    for jdx in range(len(moving_branch_to)):
                        if self.Network_solution[self.node_indices[self.decision['node_num'][0]]] - self.Network_solution[self.node_indices[node_opt[jdx]]] <= 0:
                            ...
                            self.branch_drp_time[moving_branch_to[jdx]] = np.insert(self.branch_drp_time[moving_branch_to[jdx]], 0, self.branch_drp_time[moving_branch_from][-1])
                            self.branch_drp_num[moving_branch_to[jdx]] = np.insert(self.branch_drp_num[moving_branch_to[jdx]], 0, self.branch_drp_num[moving_branch_from][-1])
                            """elif self.Branch_velocity[moving_branch_to[jdx]] < 0:
                                if len(self.branch_drp_type[moving_branch_to[jdx]]) > 1:
                                    self.branch_drp_pstn[moving_branch_to[jdx]] = np.insert(self.branch_drp_pstn[moving_branch_to[jdx]], -1, 1)
                                    self.branch_drp_type[moving_branch_to[jdx]] = np.insert(self.branch_drp_type[moving_branch_to[jdx]], -1, self.branch_drp_type[moving_branch_from][-1])
                                    self.branch_drp_time[moving_branch_to[jdx]] = np.insert(self.branch_drp_time[moving_branch_to[jdx]], -1, self.branch_drp_time[moving_branch_from][-1])
                                    self.branch_drp_num[moving_branch_to[jdx]] = np.insert(self.branch_drp_num[moving_branch_to[jdx]], -1, self.branch_drp_num[moving_branch_from][-1])
    ...
    def simulate_network_func(self):
        ...
        #print(self.exit_drps)
        #print(self.exit_drp_type)
        #print(self.exit_drp_res_time)
        #print('inlet-spacing',self.value_spacing)#source_spacing
        #print('outlet-spacing',self.exit_drp_spacing)
        #print(self.exit_drp_pstn)
        #print(self.d)
        #print(self.entrance_drp_cntr)
        #print(self.exit_drp_cntr)
    ...
    def network_fitness_calc_fn(self):
        """
