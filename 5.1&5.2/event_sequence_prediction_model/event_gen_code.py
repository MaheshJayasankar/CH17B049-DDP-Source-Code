from rl_agent_util import NodeClass
import numpy as np
import random
...
from SpacingFeasibilityNew import check_feasibility, get_velocities, set_up_network
from copy import deepcopy
import time

# Written for use in Dual Degree Project, 2021-2022 by J Mahesh, CH17B049, Dept. Chemical Engineering, IIT Madras
# The following code contains the logic used for the Event Sequence Prediction Algorithm
# The NewRunTest function at the end of this code is called externally from main.py
...
# Four major classes are defined: Event, SimulationSystem, QLearningAgent and DQNAgent.
# Event & SimulationSystem are used for constructing the simulation and events logics used in the algorithm.
# QLearningAgent and DQNAgent are used to set up the Q-Learning model.
# The main algorithm is given in NewRunTest function below the four classes.
# ====================================
...
class SimulationSystem:
    ...
    def GenerateVideo(self):
        """
        Video generation functionality disabled
        """
...
class QLearningAgent:
    """
    The base QLearningAgent class. It is mostly replaced and overridden by DQNAgent which is actually the class used by this algorithm
    """
...
class DQNAgent(QLearningAgent):
    """
    Extension of the QLearningAgent class. This separation was made due to a change in implementation mid-way through the project.
    """
    ...
    def Update(self, feascheck_result, dm_drop, dm_drop_exit_id, prev_drop_positions, new_drop_positions):
        """
        Update weights based on feasibility result, decision making droplet, preferred exit id of droplet, previous and current positions of the droplets
        """
    ...
    def Step(self, sim_env, disable_update = False):
        ...
        MAX_STEP_BEFORE_CHECK = 25

        if (self.step_count >= MAX_STEP_BEFORE_CHECK):
            lastpos = self.droplet_positions_history[-1]
            restposhist = self.droplet_positions_history[:-1]
            if (lastpos in restposhist):
                return True, False
    ...
    def PickDmDropEvent(self, sim_env):
        """
        Picks the next Decision Making Droplet given the event sequence so far
        """
        ...
        while not is_feasible:
            if (len(av_event_dict) <= 0):
                f = open("infeasible_eventseq_log.txt", "a")
...
def ConvertEventList(event_list):
    """ 
    Convert event list into a format usable by feasibility checker
    """
...
def CheckFeasibility(droplet_positions_history, dm_drop = -1):
    ...
    if (dm_drop != -1):
        if (droplet_positions_history[-2][dm_drop] == 'entrance'):
            # In this case, the event is always feasible, as it is an entrance event. So return True and a very low tolerance margin (for logging purpose)
            return True, 1e-9
    # Returns if_feasible, tol. if_feasible is a boolean indicating feasibility. tol is the minimum value of the objective function used by the solver.
...
def NewRunTest(seed_list, iter_count = 250, modify = False, new_eps = 1, new_eps_anl = False):
    """
    NewRunTest will run iterations of the event sequence algorithm for the given seed list. If modify is set to true, the epsilon parameter may be modified.
    By default, setting modify to True will result in purely random enumeration.
    """    
    ...
    for idx, run_number in enumerate(seeds):
        ...
        for iter_number in range(iter_count):
            ...
            # Repeat until all droplets exit
            while not done:
            ...
            # Calculate fitness, append stats
        ...
        # Finished all iterations for this seed.
        ed_time = time.time()
        ...
        eta_time = (run_count - idx - 1) * sum(toe_list) / len(toe_list)
    ...
    # All seeds completed. Return the results as a dict back to main.py
    print("\n=========================\n")
